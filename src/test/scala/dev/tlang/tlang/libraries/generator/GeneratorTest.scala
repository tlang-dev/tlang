...
import dev.tlang.tlang.tmpl.lang.ast.{LangBlock, TmplBlockAsValue, LangPkg, LangStringID}
...
class GeneratorTest extends AnyFunSuite {
 ...
 /* test("Simple generation") {
  ...
  test("Not existing language") {
    val block = TmplBlock(None, "myBlock", "klingon", None, Some(TmplPkg(None, List(TmplStringID(None, "myPackage")))))
    val blockAsValue = TmplBlockAsValue(None, block, Context())
