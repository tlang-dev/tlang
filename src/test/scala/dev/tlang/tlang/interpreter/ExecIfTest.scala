...
class ExecIfTest extends AnyFunSuite {
  ...
  test("If with true boolean first statement") {
    ...
    val statement = HelperIf(None, Operation(None, None, Right(CallObject(None, List(CallVarObject(None, "var1"))))), ifTrue = Some(HelperContent(None, Some(List(CallObject(None, List(CallVarObject(None, "var1"))))))))
