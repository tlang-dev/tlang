...
import io.sorne.tlang.ast.common.value.{CommonNewValue, TLangString}
import io.sorne.tlang.ast.{common, helper}
import io.sorne.tlang.ast.helper.value
...
class ExecPrimitiveValueTest extends AnyFunSuite {
  ...
  test("Simple new value") {
    val statement = value.CommonNewValue(new TLangString("myValue"))
