...
public class StyleUtils {
    ...
    public static FuncRet findStyles(Entity block, String name) {
        var styles = new List();
...
//            }
    ...
    public static FuncRet findRule(Entity struct, String name) {
        var rule = struct.getAttr(new String(name.get()));
        if (rule.isNull().get()) {
            return FuncRet.of(Array.empty());
        } else {
            return FuncRet.of(new Array(new Entity[]{(Entity) rule.get().getValue()}));
        }
    }
}
