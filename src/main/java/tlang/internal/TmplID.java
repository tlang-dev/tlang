...
public abstract class TmplID implements TmplNode<TmplID>, Value {
    ...
    public String toStr() {
        if (this instanceof TmplStringID) {
            ...
            java.lang.String pre = ((String) interpretedId.getPre().orElse(new String("")).get().getValue().getValue()).get();
            java.lang.String post = ((String) interpretedId.getPost().orElse(new String("")).get().getValue().getValue()).get();
            return new String(pre + "${uninterpreted}" + post);
        } else if (this instanceof TmplReplacedId replacedId) {
            java.lang.String pre = ((String) replacedId.getPre().orElse(new String("")).get().getValue().getValue()).get();
            java.lang.String post = ((String) replacedId.getPost().orElse(new String("")).get().getValue().getValue()).get();
