...
case class TmplVar(context: Option[ContextContent], var annots: Option[List[TmplAnnotation]] = None, var props: Option[TmplProp] = None, var name: TmplID, var `type`: Option[TmplType], var value: Option[TmplExpression]) extends TmplExpression with AstContext {
