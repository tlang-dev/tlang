...
object ExecCallObject extends Executor {
  ...
  def tryExternalResource(statements: List[CallObjectType], context: AstContext): Either[ExecError, Option[List[Value]]] = {
  ...
  private def findVar(name: String, context: AstContext): Either[ExecError, Option[List[Value]]] = {
...
//        }
  ...
  def findInEntity(name: String, entity: EntityValue, context: AstContext, caller: CallObjectType): Either[ExecError, Option[List[Value]]] = {
...
//      Right(Some(List(new TLangString(Null.empty(), entity.getType.getType.toString))))
  ...
  def findModelInEntity(name: String, entity: EntityValue, context: AstContext, caller: CallObjectType): Either[ExecError, Option[List[Value]]] = {
    entity.`type` match {
  ...
  def findModelFromType(name: String, modelType: ValueType, currentScope: Scope, context: AstContext, caller: CallObjectType): Either[ExecError, Option[List[Value]]] = {
...
//    }
  ...
  def findInModel(name: String, model: ModelSetEntity, context: AstContext, caller: CallObjectType): Either[ExecError, Option[List[Value]]] = {
    if (model.attrs.isDefined) findInSetAttrs(name, model.attrs.get, context, model.context, caller)
    else Right(None)
  }

  def findInSetAttrs(name: String, attrs: List[ModelSetAttribute], context: AstContext, contextContent: Option[ContextContent], caller: CallObjectType): Either[ExecError, Option[List[Value]]] = {
    attrs.find(_.attr.getOrElse(false).equals(name)) match {
      case Some(value) => value.value match {
        case ref: ModelSetRef =>
          caller match {
            case _: CallFuncObject => Right(Some(List(ExecUtils.modelRefToCallRefFunc(ref))))
//            case _: CallVarObject => ExecModelSetRef.run(ref, AStContext(List(ref.scope)))
          }
...
//        case _ => Left(WrongType("Should be a ref", contextContent))
  ...
  def resolveFunc(caller: CallFuncObject, callable: Option[List[Value]], context: AstContext): Either[ExecError, Option[List[Value]]] = {
...
//        case _ => Left(NotImplemented(value.getType.toString, value.getContext))
  ...
  private def execFuncWithCaller(newCaller: CallFuncObject, func: HelperFunc, context: AstContext): Either[ExecError, Option[List[Value]]] = {
    val newScope = Scope(functions = mutable.Map(newCaller.name.get -> func))
//    val newContext = Context(context.scopes :+ newScope)
    ExecCallFunc.run(newCaller, context)
  }

  private def execRefFuncWithCaller(newCaller: CallFuncObject, refFunc: CallRefFuncObject, context: AstContext): Either[ExecError, Option[List[Value]]] = {
    val newScope = Scope(refFunctions = mutable.Map(newCaller.name.get -> refFunc))
//    val newContext = Context(context.scopes :+ newScope)
    ExecCallFunc.run(newCaller, context)
  }

//  private def execFuncInEntity(caller: CallFuncObject, entity: EntityValue, context: AstContext): Either[ExecError, Option[List[Value]]] = {
...
//  }
  ...
  private def runIfOperation(result: Either[ExecError, Option[List[Value]]], context: AstContext): Either[ExecError, Option[List[Value]]] = {
