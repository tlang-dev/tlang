...
@Path("/api/favourite-todos")
@Produces("application/json")
@Consumes("application/json")
class FavouriteTodoResource {
    @Inject
    lateinit var service: FavouriteTodoService
    @GET
    ...
    @POST
    fun create(entity: FavouriteTodo): jakarta.ws.rs.core.Response {
        val created = service.create(entity)
        return jakarta.ws.rs.core.Response.status(201).entity(created).build()
    }
    @PUT
    @Path("/{id}")
    fun update(@PathParam("id") id: Long, entity: FavouriteTodo): jakarta.ws.rs.core.Response {
