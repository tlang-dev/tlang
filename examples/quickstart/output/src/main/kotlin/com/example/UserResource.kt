...
class UserResource {
    ...
    @GET
    fun list(): List<User> = User.listAll()

    @GET
    @Path("/{id}")
    fun findById(@PathParam("id") id: Long): Response =
        User.findById(id)?.let { Response.ok(it).build() }
            ?: Response.status(404).build()

    @POST
    @Transactional
    fun create(entity: User): Response {
        entity.persist()
    ...
    fun update(@PathParam("id") id: Long, entity: User): Response {
        val existing = User.findById(id) ?: return Response.status(404).build()
